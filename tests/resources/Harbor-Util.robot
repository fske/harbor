...
Package Harbor Offline
    [Arguments]  ${golang_image}=golang:${GOLANG_VERSION}  ${clarity_image}=fske/harbor-clarity-ui-builder:${CLAIR_BUILDER}  ${with_notary}=true  ${with_clair}=true  ${with_migrator}=true
    Log To Console  \nStart Docker Daemon
    Start Docker Daemon Locally
...
Compile and Up Harbor With Source Code
    [Arguments]  ${golang_image}=golang:${GOLANG_VERSION}  ${clarity_image}=fske/harbor-clarity-ui-builder:${CLAIR_BUILDER}  ${with_notary}=true  ${with_clair}=true
